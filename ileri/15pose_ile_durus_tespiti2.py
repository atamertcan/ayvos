...
class poseDetector():
    ...
    def findPoints(self, frame, draw=True):
        ...
        if self.results.pose_landmarks:
            ...
            ankle = [int(landmarks[self.mpPose.PoseLandmark.LEFT_ANKLE.value].x * w),
                        int(landmarks[self.mpPose.PoseLandmark.LEFT_ANKLE.value].y * h)]
            lmList = [hip, knee, ankle]
...
def main():
    capture = cv.VideoCapture("../Videos/runningandwalking.mp4")
    detector = poseDetector()
    angles = deque(maxlen=20)
    while True:
        ...
        if avg_angle < 160:
            state = "Running"
        elif avg_angle > 170 and state == "Running":
            state = "Walking"

        cv.putText(frame, state, (50, 75),
